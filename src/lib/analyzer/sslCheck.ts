...
/**
 * Node's PeerCertificate issuer fields (O, CN, etc.) are typed as
 * `string | string[]` because a certificate's Issuer DN can technically
 * repeat an RDN (e.g. multiple "O=" values). In practice this is extremely
 * rare, but we normalize defensively so we always store a single display
 * string rather than accidentally rendering "[object Object]" or crashing.
 */
function normalizeIssuerField(value: string | string[] | undefined): string | null {
...
export async function checkSSL(url: string): Promise<SSLResult> {
  ...
  } else if (trustedCheck.certDetails) {
    // Trusted chain — there is no cert issue to report.
    certDetails = { ...trustedCheck.certDetails, certIssue: null };
  }
        ...
        () => {
          ...
          const certDetails = cert && Object.keys(cert).length > 0
            ? {
                ...
                issuer: normalizeIssuerField(cert.issuer?.O) || normalizeIssuerField(cert.issuer?.CN) || null,
              }
            : null;
    ...
    try {
      const socket = tls.connect(
        ...
        () => {
          ...
          const issuer = normalizeIssuerField(cert.issuer?.O) || normalizeIssuerField(cert.issuer?.CN) || null;
          const now = new Date();
